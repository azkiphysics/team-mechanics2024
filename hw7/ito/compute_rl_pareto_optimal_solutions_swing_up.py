...
def execute_rl_training(
    ...
    #     Q = "_".join([f"{val:.4f}" for val in Q])
    # else:
    #     Q = f"{Q:.4f}"
    # if isinstance(R, list):
    #     R = "_".join([f"{val:.4f}" for val in R])
    # else:
    #     R = f"{R:.4f}"
    # if isinstance(Qf, list):
    #     Qf = "_".join([f"{val:.4f}" for val in Qf])
    # else:
    #     Qf = f"{Qf:.4f}"
    # savedir = os.path.join(
    #     "results", "CartPoleEnv", "SwingUp", "TD3", "scratch", "pareto_optimal_solutions", f"Q_{Q}_R_{R}_Qf_{Qf}"
    # )
...
if __name__ == "__main__":
    ...
    Qs = np.linspace(0.002, 0.01, 5)
    Rs = np.linspace(0.002, 0.01, 5)
    ...
    try:
        ...
        # データの保存
        savedir = os.path.join("results", "CartPoleEnv", "SwingUp", "TD3", "scratch", "pareto_optimal_solutions")
        os.makedirs(savedir, exist_ok=True)
